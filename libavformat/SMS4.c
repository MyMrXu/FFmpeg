...
uint32 *SMS4Decrypt2(uint32 *psrc,uint32 *src2, uint32 lgsrc)
{
	uint32 *pRet = NULL;
	uint32 i = 0;
	...
	if(psrc != NULL)
	{
		...
		/* the same arithmetic, different encryption key sequence. */
		SMS4Encrypt2(psrc,src2,lgsrc, 0);
	}

	return pRet;
}

uint32 *SMS4Encrypt2(uint32 *psrc,uint32 *src2, uint32 lgsrc,uint32 flag)
{
	uint32 *pRet = NULL;
	uint32 i = 0;
	int j = 0;
	...
	if(psrc != NULL)
	{
		...
		**======================================*/
		while (ulCnter<ulTotal) 
		{
			...
			psrc[0] = src2[0] = ulbuf[35];
			psrc[1] = src2[1] = ulbuf[34];
			psrc[2] = src2[2] = ulbuf[33];
			psrc[3] = src2[3] = ulbuf[32];
			temp = (unsigned char*)psrc;
			for (j=0 ; j < 16; j++)
			{
				*(temp+j) = (*(temp+j))>>3;
			}
			temp = (unsigned char*)src2;
			for (j=0 ; j < 16; j++)
			{
				*(temp+j) = (*(temp+j))&7;
			}
